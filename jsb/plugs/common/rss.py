...
sleeptime=15*60, running=0):
    ...
    def fetchdata(self, data=None):
        ...
        name = self.data.name
        from jsb.utils.url import enabled
    ...
    def sync(self):
        ...
        result = self.fetchdata()
        logging.info("%s entries fetched" % len(result))
        return result or []
    ...
    def updateseen(self, entries=None):
        ...
        if entries:
            for res in entries[::-1]: self.setseen(res)
        self.save(coreonly=True)
        logging.info("%s - %s items updated" % (self.data.name, len(entries)))
    ...
    def deliver(self, datalist, save=True):
        ...
        try:
            ...
            for item in loopover:
                ...
                if bot.type == "irc": logging.warn("waiting for irc bot to connect") ; bot.connectok.wait()
                res2 = datalist
    ...
    def syncdeliver(self, name):
        ...
        if feed:
            ...
            if result:
                ...
                else: logging.debug("no result from check - %s" % name)
            else: logging.debug("not result from sync - %s" % name)
        else: logging.warn("can't find %s feed" % name)
...
def doperiodicalGAE(*args, **kwargs):
    ...
    logging.warn("feeds to fetch: %s" % str(feedstofetch))
    got = False
    for f in feedstofetch:
        if not f: continue
        lastpoll.data[f] = curtime
        if f not in urls.data: logging.warn("adding %s to urls" % f) ; url = Feed(f).data.url ; urls.data[f] = url ; got = True
...
def init():
    ...
    if not runners.data: start_new_thread(watcher.checkrunners, ())
