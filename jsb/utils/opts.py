...
def makesxmppconfig(opts=None, botname=None):
    ...
    if not opts:
        ...
        cfg.openfire = False
        return cfg        
    if opts.openfire: cfg.openfire = True ; print "OpenFire mode is enabled."
    if opts.doregister: cfg.doregister = True ; print "register mode is enabled."
    else: cfg.doregister = False ; print "register mode is disabled."
    if not cfg.disable: cfg.disable = False
    if not cfg.channels: cfg.channels = []
