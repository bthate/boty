...
def setloglevel(level_name="warn", colors=False, datadir=None):
    ...
    format_short = "\033[1m%(asctime)-8s\033[0m \033[92m%(module)+12s.%(funcName)-12s\033[0m - \033[93m%(message)s\033[0m - \033[94m%(threadName)-20s\033[0m"
    format = "\033[1m%(asctime)s.%(msecs).15s\033[0m - \033[92m%(lineno)-5s%(module)+12s.%(funcName)-15s\033[0m - \033[93m%(message)s\033[0m - %(levelname)s - <%(threadName)s>"
    format_short_plain = "%(asctime)-8s %(module)+12s.%(funcName)-12s - %(message)s - %(threadName)-20s"
    format_plain = "%(asctime)s.%(msecs)-15s - %(lineno).5s%(module)+12s.%(funcName)-15s - %(message)s - %(levelname)s - <%(threadName)s>"
    datefmt = '%H:%M:%S'
