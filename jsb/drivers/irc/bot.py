...
class IRCBot(Irc):
    ...
    def _dccloop(self, sock, nick, userhost, channel=None):
        ...
        while 1:
            ...
            try:
                ...
                logging.warn("%s - dcc - %s got %s" % (self.cfg.name, userhost, res))
    ...
    def handle_mode(self, ievent):
        ...
        logging.warn("%s - mode change %s" % (self.cfg.name, str(ievent.arguments)))
        try:
    ...
    def _onconnect(self):
        ...
        if m:
            logging.warn("sending servermodes %s" % m)
