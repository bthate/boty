...
class Irc(BotBase):
    ...
    def _connect(self):
        ...
        if self.cfg.has_key('ssl') and self.cfg['ssl']:
            logging.warn('%s - ssl enabled' % self.cfg.name)
        ...
        while not self.stopped and not self.stopreadloop and self.sock and self.fsock:
            try:
                ...
                for r in intxt:
                    ...
                    if not rr: continue
                    res = rr
                    if not res.startswith("PING"): logging.warn(u"< %s (%s)" % (res, self.cfg.name))
                ...
                logging.warn("%s - socket timeout" % self.cfg.name)
                self.error = str(ex)
                ...
                if self.blocking and 'temporarily' in str(ex):
                    logging.warn("IOError: %s" % self.error)
                    time.sleep(1)
                    continue
                if not self.stopped:
    ...
    def _resume(self, data, botname, reto=None):
        ...
        try:
            ...
            print data.tojson()
            return 0
        logging.warn("%s - resume - file descriptor is %s" % (data.name, fd))
        # create socket
    ...
    def ping(self):
        ...
        try:
            ...
            logging.warn("%s - can't send ping: %s" % (self.cfg.name, str(ex)))
            return 0
