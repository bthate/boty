...
from jsb.lib.errors import CannotAuth, CantLogon
...
class SXMPPBot(XMLStream, BotBase):
    ...
    #def doreconnect(self, first=False):
        ...
        if not first:
            newbot = getfleet().makebot('sxmpp', self.cfg.name, config=self.cfg, showerror=True)
        ...
        newbot.start(connect=True)
        newbot.joinchannels()
        if not first: getfleet().replace(botjid, newbot)
        """
    #    self.connect()
    #    return True
