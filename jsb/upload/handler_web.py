...
class HomePageHandler(RequestHandler):
    ...
    def get(self):
        ...
        try:
            ...
            user = finduser()
            if user:
                start(self.response, {'appname': mainconfig.appname , 'who': user, 'loginurl': 'logged in', 'logouturl': logout, 'onload': 'consoleinit();'})
            else: 
                login(self.response, {'appname': mainconfig.appname , 'who': "nobody", 'loginurl': 'not logged in', 'logouturl': logout, 'onload': 'consoleinit();'})
