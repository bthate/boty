...
class Fleet(Persist):
    ...
    def loadall(self, names=[]):
        ...
        for name in target:
            ...
            except BotNotEnabled: logging.warn("%s is not enabled" % name) ; continue
            except KeyError: logging.error("no type know for %s bot" % name)
            except Exception, ex: handle_exception()
            if bot: self.addbot(bot)
            else: logging.error("makebot returned None")
        return bots
    ...
    def addbot(self, bot):
        ...
        for i in range(len(self.bots)-1, -1, -1):
            if self.bots[i] == bot: logging.warn("bot %s already in fleet" % repr(bot)) ; continue
            if self.bots[i].cfg.name == bot.cfg.name:
                logging.debug('removing %s from fleet' % bot.botname)
                del self.bots[i]
        self.bots.append(bot)
        logging.warn('adding %s' % bot.cfg.name)
    ...
    def startall(self, bots=None, usethreads=True):
        ...
        for bot in target:
            ...
            if usethreads: threads.append(start_new_thread(bot.reconnect, (True,))) ; continue
